...
cursor.execute('select count(*) as num_rules from transfer_rules where rule_status != 0')
print('  Reset status for {:,} rules ...'.format(cursor.fetchone().num_rules))
cursor.execute('update transfer_rules set rule_status = 0 where rule_status != 0')
...
for event in events:
  cursor.execute("""select rule_status
                      from transfer_rules
                     where id = %s
                 """, (event.rule_id,))
  rule_status = cursor.fetchone().rule_status
  # print('status is {}\n  event_type is {}\n  bitmask is {}'.format(status, event.event_type, bitmasks[event.event_type]))
  rule_status = rule_status | bitmasks[event.event_type]
  # print('new status:', status)
  cursor.execute("""
                  update transfer_rules set rule_status = %s
                   where id = %s
               """, (rule_status, event.rule_id))
  db.commit()
print('  Done')
db.close()
